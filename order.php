...
$sql = "SELECT * FROM users INNER JOIN users_orders ON users.u_id = users_orders.u_id";
...
for( $i = 0; $i < count($row); $i++) { ?>
    ...
    <td><a class="btn btn-primary btn-xs py-1 px-3"><?=$row[$i]['status']?></a></td>
    <td><a href="delete_order.php?order_del=<?=$row[$i]['o_id']?>" onclick="return confirm('Are you sure?');" class="btn px-3 py-1 btn-danger"><i class="fa fa-trash" ></i></a></td>
    
    

    <td><a class="btn btn-secondary btn-xs py-1 px-3" href="edit.php?id=<?=$row[$i]['u_id']?>">edit</a></td>
</tr>
